                            ...
                            <div class="list-group">
                                  ...
                                  <a v-for="(userDm, index) in fetchedUserDmList" :key="`userDm${index}`" :value="`userDm${index}`" @click="selectUserDm(userDm);" class="m-0 list-group-item list-group-item-action py-0">
                                      <div class="row pl-2">
                                          <div class="col-2 d-flex justify-content-center align-self-center">
                                            <img class="rounded-circle ml-2" width="auto" height="50px" style="object-fit: cover;" :src="userDm.user.profile_url || 'https://t1.daumcdn.net/qna/image/1542632018000000528'" alt="">
                                          </div>
                                          <div class="col-8">
                                              ...
                                              <div class="d-flex justify-content-between ml-2" style="position: relative; height:20px; overflow: hidden;">
                                                  ...
                                                  <span class="badge badge-primary badge-pill align-self-center" v-if="unReadCnt(userDm) > 0"> {{ unReadCnt(userDm) }}</span>
                                              </div>
                                          </div>
                                          <div class="col-1">
                              ...
                              <virtual-list :size="80" :remain="8" v-else class="bg-white">
                                    ...
                                    <div class="media-body">
                                      <div class="notification align-self-center row">
                                        <div class="col-2 d-flex justify-content-center">
                                          <img v-if="result.profile_url" :src="result.profile_url" class="rounded-circle ml-3" width="auto" height="50px" style="object-fit: cover;">
                                          <img v-else src="https://t1.daumcdn.net/qna/image/1542632018000000528" class="rounded-circle ml-3" width="auto" height="50px" style="object-fit: cover;">
                                        </div>
                                        <div class="col-8" style="position: relative; overflow: hidden;">
                                          <p class="mb-0 ml-2">{{ result.user_id }}</p>
                                          <small v-if="result.description" class="ml-2" style="position: absolute; word-break:break-all;">{{ result.description }}</small>
                                        </div>
                                ...
                                <div v-show="!inputIdData">
                                  <div class="bg-white pl-3 text-info"><small>친구목록</small></div>
                                  <div v-for="follow in fetchedFollowList" :key="follow.index" class="m-0 list-group-item list-group-item-action">
                                    <div class="row pl-2">
                                      <div class="col-2 d-flex justify-content-center p-0">
                                        <img class="rounded-circle ml-2" width="auto" height="50px" style="object-fit: cover;" :src="follow.profile_url || 'https://t1.daumcdn.net/qna/image/1542632018000000528'" alt=""> 
                                      </div>
                                      <div class="col-8" style="position: relative; overflow: hidden;">
...
export default {
  ...
  methods: {
    ...
    unReadCnt(userDm) {
      let cnt = 0
      http
        .get('/directMessage/unReadCnt', userDm)
        .then(response => {
          cnt = response.data
  ...
  created() {
    this.socket = io('http://192.168.100.48:3000');
